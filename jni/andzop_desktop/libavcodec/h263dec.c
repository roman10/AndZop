...
retry:
    ...
    if (CONFIG_WMV2_DECODER && s->msmpeg4_version==5) {
        ...
        //printf("ff_mpeg4_decode_picture_header\n");
...
//        dsputil_init(&s->dsp, avctx);
    ...
    if (   s->width  != avctx->coded_width
        || s->height != avctx->coded_height) {
	//printf("s->width, s->height = %d, %d;  avctx->coded_width, avctx->coded_height = %d, %d\n", s->width, s->height, avctx->coded_width, avctx->coded_height);
	if (avctx->allow_selective_decoding == 0) {
    ...
    if (!s->context_initialized) {
	//printf("avcodec_set_dimensions, and then retry\n");
    ...
    //[CLEAR]free s->current_picture
    //memset(s->current_picture.data[0], 0x00, s->current_picture.linesize[0]);
    //memset(s->current_picture.data[1], 0x00, s->current_picture.linesize[1]);
    //memset(s->current_picture.data[2], 0x00, s->current_picture.linesize[2]);
    //memset(s->current_picture.data[3], 0x00, s->current_picture.linesize[3]);

    //memset(s->last_picture.data[0], 0x00, s->last_picture.linesize[0]);
    //memset(s->last_picture.data[1], 0x00, s->last_picture.linesize[1]);
    //memset(s->last_picture.data[2], 0x00, s->last_picture.linesize[2]);
    //memset(s->last_picture.data[3], 0x00, s->last_picture.linesize[3]);

    //memset(s->next_picture.data[0], 0x00, s->next_picture.linesize[0]);
    //memset(s->next_picture.data[1], 0x00, s->next_picture.linesize[1]);
    //memset(s->next_picture.data[2], 0x00, s->next_picture.linesize[2]);
    //memset(s->next_picture.data[3], 0x00, s->next_picture.linesize[3]);

    //memset(s->new_picture.data[0], 0x00, s->new_picture.linesize[0]);
    //memset(s->new_picture.data[1], 0x00, s->new_picture.linesize[1]);
    //memset(s->new_picture.data[2], 0x00, s->new_picture.linesize[2]);
    //memset(s->new_picture.data[3], 0x00, s->new_picture.linesize[3]);
    ...
    //*data_size = sizeof(AVFrame);
    //return get_consumed_bytes(s, buf_size);
    //memset(s->current_picture_ptr->data[0], 0x00, s->current_picture_ptr->linesize[0]);
    //memset(s->current_picture_ptr->data[1], 0x00, s->current_picture_ptr->linesize[1]);
...
/*this method decode the dependency of a frame, it doesn't have to decode every mb*/
