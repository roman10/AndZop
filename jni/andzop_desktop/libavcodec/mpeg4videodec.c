...
int mpeg4_decode_video_packet_header(MpegEncContext *s)
{
    ...
    #undef printf
    //printf("mpeg4_decode_video_packet_header\n");
...
/**
 * decodes the dependency of a block.
 * @return <0 if an error occurred
 */
...
static int mpeg4_decode_mb_dep(MpegEncContext *s,
                      DCTELEM block[6][64])
{
	...
	/*P-VOP and S-VOP*/
        ...
        fprintf(s->avctx->g_interDepF, "%d:%d:\n", s->mv[0][0][0], s->mv[0][0][1]);
#endif
	...
	//according to H.263 doc, data for each mb consists of a macroblock header followed by data for blocks
	//COD: coded macroblock indication. set to 0 signals the mb is coded. only present for pictures that are not "INTRA", so 			not here
	//MCBPC: MB type & Coded Block Pattern for Ch: present when indicated by COD or picture is "INTRA", always appear in 			coded mb
        do{
            cbpc = get_vlc2(&s->gb, ff_h263_intra_MCBPC_vlc.table, INTRA_MCBPC_VLC_BITS, 2);
            if (cbpc < 0){
...
static int decode_vop_header(MpegEncContext *s, GetBitContext *gb){
    ...
    unsigned int debugBits;

    //printf("decode_vop_header: s->mb_height: %d; s->mb_width: %d\n", s->mb_height, s->mb_width);
    ...
    if(s->partitioned_frame)
        ...
        s->decode_mb_dep = mpeg4_decode_mb_dep;

    time_incr=0;
    #undef printf
    while ((debugBits = get_bits1(gb)) != 0) {
	//printf("1 bit, modulo_time_base: %d\n", debugBits);
        time_incr++;
    }

    #undef printf
    //printf("1 bit, marker_bit before vop_time_increment\n");
    ...
    //printf("%d bits, time_increment = %d\n", s->time_increment_bits, time_increment);
    ...
    //printf("1 bit, marker_bit\n");
    check_marker(gb, "before vop_coded");

    /* vop coded */
    if ((debugBits = get_bits1(gb)) != 1){
        if(s->avctx->debug&FF_DEBUG_PICT_INFO)
            av_log(s->avctx, AV_LOG_ERROR, "vop not coded\n");
        return FRAME_SKIPPED;
    }
    //printf("1bit, vop_coded bit: %d\n", debugBits);

    if (s->shape != BIN_ONLY_SHAPE && ( s->pict_type == FF_P_TYPE
                          || (s->pict_type == FF_S_TYPE && s->vol_sprite_usage==GMC_SPRITE))) {
        /* rounding type for motion estimation */
        s->no_rounding = get_bits1(gb);
	//printf("1 bit, vop_rounding_type: %d\n", s->no_rounding);
    } else {
        s->no_rounding = 0;
    }
...
//FIXME complexity estimation stuff
     ...
     if (s->shape != BIN_ONLY_SHAPE) {
	 //printf("%d bits skipped, complexity estimation header\n", s->cplx_estimation_trash_i);
         skip_bits_long(gb, s->cplx_estimation_trash_i);
         if(s->pict_type != FF_I_TYPE)
            skip_bits_long(gb, s->cplx_estimation_trash_p);
         if(s->pict_type == FF_B_TYPE)
            skip_bits_long(gb, s->cplx_estimation_trash_b);
	 debugBits = show_bits(gb, 3);
	 //printf("3 bits, intra_dc_vlc_threshold: %d\n", debugBits);
	 ...
	 //printf("%d bits, quantization scale: %d\n", s->quant_precision, s->chroma_qscale);
         if(s->qscale==0){
...
int ff_mpeg4_decode_picture_header(MpegEncContext * s, GetBitContext *gb)
{
    ...
    //#undef printf
    //printf("ff_mpeg4_decode_picture_header: %d, %d\n", s->mb_height, s->mb_width);
