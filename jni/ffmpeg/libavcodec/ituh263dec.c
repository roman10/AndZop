...
int h263_decode_motion(MpegEncContext * s, int pred, int f_code)
{
    ...
    code = get_vlc2(&s->gb, mv_vlc.table, MV_VLC_BITS, 2);	//mv_data
    ...
    if (sign)
        val = -val;
    val += pred;		//plus the predication value
                      ...
                      DCTELEM block[6][64])
{
    ...
    if (s->pict_type == FF_P_TYPE) {
        do{
            ...
            cbpc = get_vlc2(&s->gb, ff_h263_inter_MCBPC_vlc.table, INTER_MCBPC_VLC_BITS, 2);
            if (cbpc < 0){
                av_log(s->avctx, AV_LOG_ERROR, "P frame: cbpc damaged at %d %d\n", s->mb_x, s->mb_y);
                printf("P frame: cbpc damaged at %d %d\n", s->mb_x, s->mb_y);
                return -1;
            }
        }while(cbpc == 20);
...
//        const int mv_xy= s->mb_x + 1 + s->mb_y * s->mb_stride;
        ...
        do{
            ...
            if (mb_type < 0){
                ...
                printf("b mb_type damaged at %d %d\n", s->mb_x, s->mb_y);
                return -1;
            }
...
//FIXME UMV
        ...
        do{
            ...
            if (cbpc < 0){
                av_log(s->avctx, AV_LOG_ERROR, "I frame: cbpc damaged at %d %d\n", s->mb_x, s->mb_y);
                printf("I frame: cbpc damaged at %d %d\n", s->mb_x, s->mb_y);
                return -1;
            }
        }while(cbpc == 8);
...
int h263_decode_picture_header(MpegEncContext *s)
{
    ...
    #undef printf
    //printf("h263_decode_picture_header\n");
    align_get_bits(&s->gb);
