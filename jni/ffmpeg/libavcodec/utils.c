...
int avcodec_default_get_buffer(AVCodecContext *s, AVFrame *pic){
    ...
    //#undef printf
    //printf("avcodec_default_get_buffer\n");
    if(pic->data[0]!=NULL) {
    ...
    if(buf->base[0]){
 	...
 	//printf("before avcodec_align_dimensions2: %d, %d\n", w, h);
        avcodec_align_dimensions2(s, &w, &h, stride_align);
 	//printf("after avcodec_align_dimensions2: %d, %d\n", w, h);

        if(!(s->flags&CODEC_FLAG_EMU_EDGE)){
            w+= EDGE_WIDTH*2;
            h+= EDGE_WIDTH*2;
	    //printf("added edge width and height: %d, %d, %d\n", w, h, EDGE_WIDTH);
        }
	    ...
	    //printf("picture.linesize[0] = %d, picture.linesize[1] = %d, picture.linesize[2] = %d, picture.linesize[3] = %d, w = %d\n", picture.linesize[0], picture.linesize[1], picture.linesize[2], picture.linesize[3], w);
        ...
        //printf("av_image_fill_pointers: %d\n", tmpsize);
        if (tmpsize < 0)
            return -1;
        ...
        for(i=0; i<4 && size[i]; i++){
            ...
            //printf("buf->linesize[i] = %d, allocate memory %d\n", buf->linesize[i], size[i] + 16);
            buf->base[i]= av_malloc(size[i]+16); //FIXME 16
            //printf("buf->linesize[i] = %x, buf->base[i] = %x\n", buf->linesize[i], buf->base[i]);
            if(buf->base[i]==NULL) { 
                //feipeng: cannot allocate memory
    ...
    //printf("end of avcodec_default_get_buffer");
    return 0;
}
